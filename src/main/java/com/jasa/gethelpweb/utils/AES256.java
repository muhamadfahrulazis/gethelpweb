...
//        System.out.println("ini tmp subkey :" + tmp.toString());
        return tmp;
    }
    ...
    // Perkalian XOR antara plain text dan chiper text
    ...
    // setelah proses AddRoundkey()
    // Hasil hexadecimal di transformasikan dengan tabel sBox
    private static byte[][] SubBytes(byte[][] state) {
        ...
        for (int row = 0; row < 4; row++){
            System.out.println("ini row " + row);
            for (int col = 0; col < Nb; col++){
                System.out.println("col " + col);
                tmp[row][col] = (byte) (sbox[(state[row][col] & 0x000000ff)] & 0xff);

                System.out.println("state rowcol " + tmp[row][col]);
            }
        }
    ...
    private static byte[][] ShiftRows(byte[][] state) {
        ...
        System.out.println("Start ShifRows ");
        byte[] t = new byte[4];
        for (int r = 1; r < 4; r++) {
            System.out.println("round : " + r);
            for (int c = 0; c < Nb; c++){
                t[c] = state[r][(c + r) % Nb];
                System.out.println("ini t[c] : " + t[c]);
            }

            for (int c = 0; c < Nb; c++){
                state[r][c] = t[c];
                System.out.println("ini state [r] : " + state[r][c]);
            }
        }
        System.out.println("End Process ShiftRows ");
        return state;
    }
    ...
    public static byte[] encryptBloc(byte[] in) {
        ...
        for (int round = 1; round < Nr; round++) {
            System.out.println("ini round block : " + round);
    ...
    public static String encrypt(String text){
        ...
        return Base64.getEncoder().encodeToString(tmp).replaceAll("/", "!");
    }

    public static String decrypt(String enc){
//		1a25s8fe5dsg65ad, qwrt

        String k = "qwrt";
        byte[] key = k.getBytes();

        byte[] in = Base64.getDecoder().decode(enc.replaceAll("!", "/"));
